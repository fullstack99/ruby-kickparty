...
if (document && document.URL.indexOf('events/new') !== -1) {
  var formState = {
    VALIDATIONS: {
      ...
      contactPhone: {
        isRequiredMessage: 'Contact Phone is required',
        isRequired: true,
      ...
      contactEmail: {
        isRequiredMessage: 'Contact Email is required',
        isRequired: true,
...
var ResourcesController = function ResourcesController(id, node, cb) {
  ...
  this.cost = this.node.querySelector('.resource-cost');
...
ResourcesController.prototype.preload = function (resources) {
...
ResourcesController.prototype.renderResources = function () {
  ...
  for (var i = 0, j = this._resources.length; i < j; i++) {
    ...
    var colThree = Utils.nodeFactory('div', ['col-md-1'], '<div class="price">' + Utils.formatCurrency(resource.cost) + '</div>');

    var descHeading = Utils.nodeFactory('h5', null, resource.name + ' ');
    var description = Utils.nodeFactory('p', null, resource.description);
...
ResourcesController.prototype.updateTotal = function () {
  ...
  for (var i = 0, j = this._resources.length; i < j; i++) {
    var cost = Utils.tryParseFloat(this._resources[i].cost);
  ...
  totalNode.textContent = this.resourceTotal > 0 ? Utils.formatCurrency(this.resourceTotal) : '';
};
  ...
  if (document.URL.indexOf('resources') !== -1) {
    ...
    (function () {
      ...
      var calcTotals = function calcTotals() {
        ...
        costTotalNode.textContent = Utils.formatCurrency(tempTotal);

        // COST PP
        if (pricingMethod === 2) {
          var flatPPP = Utils.tryParseFloat(flatPriceNode.value);

          if (flatPPP) {
            costPPNode.textContent = Utils.formatCurrency(flatPPP);
          }
        } else {
          costPPNode.textContent = Utils.formatCurrency(tempTotal / peopleAttending);
        }
      };
      ...
      pricingMethods = document.querySelectorAll('.price-method');


      for (var i = 0, j = pricingMethods.length; i < j; i++) {
        pricingMethods[i].onchange = function (ev) {
          pricingMethod = ~ ~ev.target.value;

          if (~ ~ev.target.value === 2) {
            flatPriceNode.style.display = 'block';
          } else {
            flatPriceNode.style.display = 'none';
      ...
      flatPriceNode.onchange = function () {
        calcTotals();
      };

      costBaseNode.onchange = function (ev) {
        calcTotals();
      };
