$(document).ready(function () {
  ...
  const submitFormButton = $('#payment-submit-btn')
  ...
  inputs.number.blur(function (e) {
    const displayedCardType = $('.cc-brand')
  ...
  inputs.cvc.blur(function (e) {
    const cardType = $.payment.cardType(inputs.number.val())
    $(this).toggleInputError(!$.payment.validateCardCVC(this.value, cardType))
  })

  inputs.exp.blur(function (e) {
    const expiration = $(this).payment('cardExpiryVal')
  ...
  paymentForm.submit(function (e) {
    ...
    const inputsArray = $.map(inputs, input => [input]) // useful to map/filter/etc over
    const emptyInputs = inputsArray.filter(input => !input.val())
